...
5. Repeat with a BIOS + LSI Logic + e1000 Windows Server VM choosing BYOL; confirm `launchOptions.firmware=BIOS`, `bootVolumeType=PARAVIRTUALIZED`, `networkType=PARAVIRTUALIZED` (the source device model does not matter; only *Maximum compatibility* yields IDE + E1000), `licensingConfigs=[WINDOWS/BRING_YOUR_OWN_LICENSE]`, then change the license type from the job view and verify in the console.
