...
- **Secure Boot** on the source (`efiSecureBootEnabled`) is matched: the instance is launched as a *shielded instance* with Secure Boot on, from a seed image whose capability schema declares `Compute.SecureBoot`. This needs an x86 shape (AMD E-series, Intel Standard3/Optimized3, or bare metal); Ampere A1/A2 shapes are refused up front. Measured Boot and the vTPM are not enabled. The guest must boot through a boot loader signed for the Microsoft UEFI CA (shim-based Linux distributions, Windows); a guest that booted with custom Secure Boot keys on vSphere will not start in OCI.
...
| `target instance entered state TERMINATING` | OCI accepted the launch and failed it asynchronously; the job error now appends the reason from the instance's work request (needs `read work-requests`). DNS host names are unique per subnet: the helper checks the subnet's private IPs before launching and uses `name-2`, `name-3`, ... when the VM name is already taken. |
| `cannot open /dev/oracleoci/oraclevdX` / `cannot open /dev/sdX` | Helper container needs `--privileged`, `/dev` and `/sys` mounted; instance must use consistent device naming (data volumes). |
| `boot volume attached to the helper but its disk was not found` | The boot volume was attached without a device path (OCI does not allow one) and no new whole disk of the boot volume size showed up in `/sys/block` within `HELPER_VOLUME_TIMEOUT_S`. Check `lsblk` on the helper and that nothing else attaches volumes to it at the same time. |
...
| Export throughput is low (~15-25 MB/s) | By default the disk stream is proxied by vCenter, which caps around that rate. Tick *Download the disks directly from the ESXi host* in the migration dialog (helper must reach the host shown there on TCP 443 and resolve its name). Also check the VPN/FastConnect bandwidth; a higher *Volume performance (VPU/GB)* in the same dialog raises the write ceiling of the target volumes, and *Decode and write on a separate thread* overlaps the download with inflate/`pwrite` (helps when the helper's CPU or the volume, not the network, is the bottleneck). Per disk the stream is a single TCP flow compressed by one ESXi thread, so multi-disk VMs are the only case where more parallelism would help. |
